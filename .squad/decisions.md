...
## Active Decisions (Current Session)
...
A minimal-cost Ubuntu demo VM (`vm-amlab-<uid>`, Standard_B2ats_v2) was deployed to `rg-amlab` with guest metrics collection via AMA bound to a custom DCR feeding `law-amlab-<uid>`. All resources additive only. **Critical finding:** AMA requires `identity: { type: 'SystemAssigned' }` on the VM resource for IMDS MSI token flow.

*Full details in `.squad/decisions-archive/decisions-2026-07-16T10-13-53-05-00Z-archived.md`*

---

### Decision: Mouse — Notes Rewrite (1–38) + Static Page-Number Fix (true 1..50)

**Date:** 2026-07-17  
**Author:** Mouse (Content & Docs)  
**Status:** COMPLETE  
**Consumers:** All team members, @dmauser  
**Requested by:** Daniel Mauser (@dmauser) — both tasks pre-approved

**Task 1 — Speaker notes rewritten on the original 38 slides.**  
Converted the single-paragraph narrative notes on the 38 original slides (final positions 1–14, 16–22, 24–34, 36–41) into the same bullet "bar" used on the demo slides 39–45: a lead framing line (existing timing tag preserved) + 3–6 concise presenter talking points each. Every point is grounded in the slide's own title/body and the prior note substance — no invented product facts, numbers, or preview claims. Dividers (5/16/24/36) kept light (3 bullets). The 5 references slides (15/23/35/47/50) remain note-free by design. Demo slides 42–46/48/49 already met the bar and were left untouched.

**Task 2 — Baked-in static page numbers corrected to true 1..50.**  
The footer page numbers are static text runs (shape at left=11292840, top=6437376 EMU), which had drifted after the 5 refs slides were inserted mid-deck (e.g. slide 17 read "16"). Each footer run was set to its true 1-based final position, changing only that one run. All 44 footer numbers were STATIC text — zero real slide-number fields (confirmed by XML inspection for `<a:fld slidenum>`; the drift itself also proves static). 6 slides have no footer page-number shape and were left alone: pos 1 (title), 5/16/24/36 (dividers), 34 (Next Steps).

**QA outcome:**  
- **Notes coverage:** 45/45 content slides carry 3–6 bullet talking points (dividers = 3). PASS.
- **Page numbers:** every slide with a footer shape reads its own position; sequence is 1..50. PASS.
- **Regression:** visible body text (excluding the footer page-number run), whitespace-normalized, 0 mismatches across all 50 slides vs the pre-edit backup. PASS.
- **Static-vs-field:** 44 static (updated where drifted), 0 fields; 6 slides have no footer number.
- **Visual QA:** spot-rendered pos 16 (divider, no number), 17 (16→17), 27 (dense body), 47 (refs) — corrected numbers render, footer/theme intact, nothing shifted. PASS.
- **Count/PDF:** 50 slides; PDF re-exported (50 pages, 2026-07-17 10:03).

**Files:**  
- `docs/Azure-Monitor-Observability-Workshop.pptx` (50 slides)
- `docs/Azure-Monitor-Observability-Workshop.pdf` (50 pages, re-exported)

---

### Decision: Mouse — Speaker Notes Rewrite (1–38) + Doc Grounding + Page-Number Correction
...
**TASK 1: Notes Rewrite (1–38 + Doc Grounding)**  
Retrofitted speaker notes on slides 1–38, excluding the refs slides at 15/23/35 → **35 slides** rewritten. Each in-scope slide now carries:
- One explicit "Doc anchor:" talking point stating what the Azure Monitor feature ACTUALLY is per official docs
- Closing "Ref: <learn.microsoft.com URL>" bullet  
- All 20 distinct URLs reused from the 5 already-verified references slides (15/23/35/47/50) — zero new URLs, all HTTP 200  
- No invented limits/SLAs/pricing; softened two prior specifics to conceptual form (e.g., "commitment tiers at 100 GB/day" → "for high-volume workspaces"; "MMA retired Aug 2024" → "legacy MMA is retired — migrate to AMA")

**QA outcome (Task 1):**
- Notes coverage: all 35 in-scope slides = 4–5 bullets (within 3–6 budget), each with `Ref:` line, domain learn.microsoft.com. **PASS.**
- URL integrity: 20/20 distinct URLs → HTTP 200. None dropped. **PASS.**
- Regression: visible-text mismatches across all 50 slides = **0**. **PASS.**

**TASK 2: Page-Number Correction (1..50)**  
The footer page numbers are static text runs (shape at left=11292840, top=6437376 EMU), which had drifted after the 5 refs slides were inserted mid-deck (e.g. slide 17 read "16"). Each footer run was corrected to its true 1-based final position; only that one run was changed. All 44 footer numbers were static (zero real slide-number fields; confirmed by XML inspection for `<a:fld slidenum>`). 6 slides have no footer page-number shape: pos 1 (title), 5/16/24/36 (dividers), 34 (Next Steps).

**QA outcome (Task 2):**
- Static-vs-field: 44 static (updated where drifted), 0 fields; 6 slides have no footer number. **PASS.**
- Sequence: every slide with a footer shape reads its own position; sequence is 1..50. **PASS.**
- Visual QA: spot-rendered pos 16 (divider, no number), 17 (16→17), 27 (dense body), 47 (refs) — corrected numbers render, footer/theme intact, nothing shifted. **PASS.**

**Files:**  
- `docs/Azure-Monitor-Observability-Workshop.pptx` (50 slides, notes updated, page numbers corrected)
- `docs/Azure-Monitor-Observability-Workshop.pdf` (50 pages, re-exported 2026-07-17 10:18)

---

### Decision: Tank — Service Health Alert IaC + Live Deploy

**Date:** 2026-07-17T15:23:48-05:00  
**Author:** Tank (Infra / IaC Dev)  
**Requested by:** Daniel Mauser (@dmauser)

Five files were created (ADDITIVE ONLY — no existing resources modified):

| File | Purpose |
|---|---|
| `alerts/service-health.alerts.bicep` | Bicep module: action group + activity log alert |
...
`alerts/service-health.alerts.bicep` deploys two resources at `resourceGroup` scope:

1. **`microsoft.insights/actionGroups@2019-06-01`** — named `ag-${namePrefix}-service-health`, location `Global`, one `emailReceiver` wired to the `emailAddress` param.
2. **`microsoft.insights/activityLogAlerts@2017-04-01`** — named `alert-${namePrefix}-service-health`, location `Global`, scopes = `[subscription().id]`, condition `category == ServiceHealth`, enabled.

Params: `emailAddress` (required), `namePrefix` (default `amlab`), `actionGroupShortName` (default `amlabSvcHlth`, ≤12 chars).  
Outputs: `actionGroupId`, `alertId`, `actionGroupName`, `alertName`.

**Why `category = ServiceHealth` + Global Action Group**

- **`category = ServiceHealth`** is the canonical filter for Azure Service Health events in the Activity Log. A single condition captures all sub-types: Incidents, Planned Maintenance, Health Advisories, and Security Advisories. Source: [Microsoft Learn — Create activity log alerts on service notifications using Bicep](https://learn.microsoft.com/en-us/azure/service-health/alerts-activity-log-service-notifications-bicep).
- **`location: Global`** is mandatory for both the action group and the activity log alert. Service Health is a subscription-level, globally-scoped signal; Azure only accepts `Global` as the resource location for these resource types.
- **`scopes: [subscription().id]`** — Activity Log alerts for Service Health must scope to the subscription (not a resource group) because Service Health events are emitted at the subscription level.

**$0 Cost Confirmation**

- **Activity Log alerts:** free (no per-alert charge).
- **Action group email notifications:** effectively free (first 1,000 emails/month free; well within lab usage).
- Reference: Azure Monitor pricing — Activity Log alerts are in the free tier.
...
**Timestamp:** `2026-07-17T21:04:59Z`

**Activity Log Alert — `az resource show` output:**
```json
{
  ...
  "properties": {
    "actions": {
      "actionGroups": [
        {
          "actionGroupId": "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-amlab/providers/microsoft.insights/actionGroups/ag-amlab-service-health"
        }
      ]
    },
    "condition": {
...
**Action Group — `az resource show` output:**
```json
{
  ...
  "properties": {
    "emailReceivers": [
      { "emailAddress": "you@example.com", "name": "amlab-email", "status": "Enabled" }
    ],
    "enabled": true,
    "groupShortName": "amlabSvcHlth"
  },
...
- `az bicep build --file alerts\service-health.alerts.bicep` → exit 0 ✅
- `az bicep lint --file alerts\service-health.alerts.bicep` → exit 0 ✅
- Schema source validated via Microsoft Learn MCP: https://learn.microsoft.com/en-us/azure/service-health/alerts-activity-log-service-notifications-bicep

---

### Decision: Mouse — Service Health Content (Doc + Slide + Refs + Notes + Page# + README + PDF)
...
**1. Walkthrough Doc**  
`docs/service-health-alerts-walkthrough.md` — new. Matches observability-agent-walkthrough.md structure (front-matter, What It Is, Prerequisites table, numbered setup paths). Covers:
- 3 Service Health components (Azure Status, Service Health, Resource Health)
- Portal hands-on path (Service Health → Create alert → Scope/Condition/Actions/Details tabs)
- IaC/CLI path (alerts/service-health.alerts.bicep + deploy script + validation command)
- All-Services/All-Regions tip, Global action-group requirement, RBAC prereqs, $0 cost
- Grounded in 8 official facts; cites 5 verified learn.microsoft.com URLs

**2. Deck — New Slide at Position 50**  
Deck: `docs/Azure-Monitor-Observability-Workshop.pptx` — 50 → 51 slides. New "Service Health & Alerting" slide inserted at position 50 (after Cost slide 49, before refs slide 50→51). Theme-matched: blue oval, Georgia 24pt title, Calibri 11pt eyebrow, 3 component cards with accent bars (blue/teal/amber), right-column event classes + key facts + setup paths. Footer + page number.

**3. Refs Slide Extended (Position 51)**  
Added SERVICE HEALTH section to the final references slide (teal accent bar + 5 hyperlinked URLs: overview, portal alerts, bicep alerts, resource-health, action-groups). Title updated to "Observability Agent & Cost & Service Health — References".

**4. Speaker Notes on Slide 50**  
7 lines (1 lead + 6 bullets): transition-in, doc anchor (3 tiers), key setup point (category=ServiceHealth + Global), demo cue, segue, Ref line. Injected via XML (notes body placeholder template from existing slide).

**5. Page Numbers Re-fixed to 1..51**  
1 number changed (old refs slide 50→51). All 45 footer-bearing slides read correct positions. 6 no-footer slides unchanged (1, 5, 16, 24, 34, 36).

**6. README Link**  
Added under existing "New:" lines at ~line 103.

**7. PDF Re-exported**  
51 pages, timestamp 2026-07-17 16:13, via PowerPoint COM SaveAs(pdf, 32).

**QA Outcome**

| Check | Result |
|---|---|
| Slide count | 51 slides. New at pos 50, refs at 51. **PASS** |
| Regression | 0 mismatches on original 50 slides' visible text (excl. page numbers). **PASS** |
| Page numbers | 45/45 correct (1..51). **PASS** |
| Notes (slide 50) | 7 lines, 6 bullets, Ref line present. **PASS** |
| Refs slide 51 | 10 Service Health hyperlinks (5 label + 5 URL). **PASS** |
| PDF | 51 pages, fresh timestamp. **PASS** |
| README | Link added. **PASS** |

**Files Modified**
- `docs/Azure-Monitor-Observability-Workshop.pptx` (51 slides)
- `docs/Azure-Monitor-Observability-Workshop.pdf` (51 pages)
- `docs/service-health-alerts-walkthrough.md` (new)
...
All prior decisions from 2026-07-15 and earlier have been archived to `.squad/decisions-archive/` due to file size (> 20 KB). Refer to the archive for:

- Trinity — Scaffold + Core Infra (2026-07-15)
- Trinity — Roles + Outputs (2026-07-15)
- Dozer — APM errorrate Determinism Fix (2026-07-15)
- Dozer — Generator Contract (2026-07-15)
- Ghost — Seed / Reseed / Smoke-Test Script Contract (2026-07-15)
- Ghost — Test Suite Inventory (2026-07-15)
- Mouse — Docs Inventory (2026-07-15)
- Mouse — KQL Column Contracts (2026-07-15)
- Coordinator — Deployment verification + PS 5.1 encoding hardening (2026-07-16)
